...
1. **Fine-tuning ruBERT/ruRoBERTa** вместо frozen encoder — на полном датасете даст F1 0.85+
2. **Выровнять дисбаланс в авторазметке** перед AL — стратифицированный sampling при формировании labeled pool
3. **Увеличить n_start** для AL — при 500 примерах из 2000 дисперсия метрик слишком высока для надёжных выводов; рекомендуется n_start ≥ 1000
